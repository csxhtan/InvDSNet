...
class attention_resnet(nn.Module):
    ...
    def forward(self, x):
        residual = self.relu1(self.conv1(x))
        residual = self.relu1(self.conv2(residual))
        out = self.relu1(self.conv3(residual))
        out_1 = self.relu1(self.conv4(out))
        out_mask = self.conv5(out_1)
        out_mask_loss = self.conv6(out_mask)
        attention_map = self.sigmoid(out_mask_loss)
